...
1. **联调依赖 MySQL**：需 `npm run db:setup` 后三端同时运行验证
2. **微信生产**：AppID/Secret 与支付 v3 未配置
3. **小程序 PRD 差距**：手机号绑定、教练页、优惠券、训练历史、真实支付
4. **文档历史**：下方「设计分析」为 2024 年初期快照，多数问题已在 Iteration 1–4 解决，仅供参考
